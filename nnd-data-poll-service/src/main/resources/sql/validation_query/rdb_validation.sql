...
SELECT 'PERTUSSIS_SUSPECTED_SOURCE_FLD' AS table_name, COUNT(*) AS count_value
...
WHERE PERTUSSIS_SUSPECTED_SOURCE_FLD.add_time <= @timestamp
   OR INVESTIGATION.LAST_CHG_TIME <= @timestamp

UNION ALL

SELECT 'PERTUSSIS_TREATMENT_FIELD' AS table_name, COUNT(*) AS count_value
FROM PERTUSSIS_TREATMENT_FIELD
         ...
         JOIN INVESTIGATION
              ON INVESTIGATION.INVESTIGATION_KEY = PERTUSSIS_CASE.INVESTIGATION_KEY
WHERE PERTUSSIS_TREATMENT_FIELD.add_time <= @timestamp
   OR INVESTIGATION.LAST_CHG_TIME <= @timestamp

UNION ALL

SELECT 'HEP_MULTI_VALUE_FIELD_GROUP' AS table_name, COUNT(*) AS count_value
...
WHERE HEP_MULTI_VALUE_FIELD_GROUP.add_time <= @timestamp
   OR INVESTIGATION.LAST_CHG_TIME <= @timestamp
