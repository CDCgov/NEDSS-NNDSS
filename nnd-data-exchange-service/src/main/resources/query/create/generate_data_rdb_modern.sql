...
VALUES
    ...
    'SELECT * FROM rdb_modern.nrt_observation WHERE last_chg_time >= :timestamp OR refresh_datetime >= :timestamp;',
    NULL,
    'SELECT COUNT(*) FROM rdb_modern.nrt_observation WHERE last_chg_time >= :timestamp OR refresh_datetime >= :timestamp;',
    'WITH PaginatedResults AS (
        SELECT *, ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS RowNum
        FROM rdb_modern.nrt_observation
